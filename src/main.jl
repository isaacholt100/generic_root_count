...
function base_coefficient_rings(F::Vector{<:MPolyRingElem})
    R = parent(first(F))
    A = coefficient_ring(R)
    return (R, A)
end

function collect_terms(F::Vector{<:MPolyRingElem})
...
function factorize_collected_terms(F::Vector{<:MPolyRingElem})
...
function modified_support(F::Vector{<:MPolyRingElem})
...
function nonlinear_resonator_system(n::Int, m::Int; specialized::Bool = false)
    if specialized
        ...
        A, ab = polynomial_ring(QQ, ab_string)
        a = ab[1:div(length(ab), 2)]
        b = ab[div(length(ab), 2)+1:end]
    end

    R, (u, v) = polynomial_ring(A, ["u", "v"])
...
function generic_root_count(system::Vector{<:MPolyRingElem})
...
n = 2
m = 3
@assert generic_root_count(nonlinear_resonator_system(n, m)) == 2 * m * n + 1
